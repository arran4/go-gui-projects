...
[gamen](https://github.com/rajveermalviya/gamen) (Requires CGO) 是 Go 语言跨平台的 GUI 窗口创建和管理库。
...
[glfw](https://github.com/go-gl/glfw) (Requires CGO) 是 GLFW 3 的 Go 绑定。

[go-fltk](https://github.com/pwiecz/go-fltk) (Requires CGO) 是一个简单的包装器 [FLTK 1.4 library](https://www.fltk.org/), 它是一个轻量级的 GUI 库，允许创建小型、独立且快速的 GUI 应用程序。

[go-gtk](https://github.com/mattn/go-gtk) (Requires CGO) 是 GTK2 的 Go 绑定。
* [RenderView](https://github.com/TheGrum/renderview) 是一个简单的 Go GUI 包装器，用于交互式操作可视化算法/后端代码。支持 go-gtk（默认）、gotk3 和 shiny 后端。

[go-mobile](https://github.com/golang/mobile) (Requires CGO) 增加了对移动平台（Android 和 iOS）的支持，并提供了构建移动应用程序的工具。(包括[Go bindings for OpenGL ES 2.0 and ES 3.0](https://godoc.org/golang.org/x/mobile/gl))

[go-sdl2](https://github.com/veandco/go-sdl2) (Requires CGO) 是专为 Go 用户包装的 SDL2。

[go-sfml](https://github.com/teh-cmc/go-sfml) (Requires CGO) 是 [SFML](https://www.sfml-dev.org/)的 Go 绑定。

[gothic](https://github.com/nsf/gothic) (Requires CGO) 是 tcl/tk 的 Go 绑定。

[gotk3](https://github.com/gotk3/gotk3) (Requires CGO) 为 GTK+3 和​​相关项目 (包括 [cairo](https://github.com/gotk3/gotk3/tree/master/cairo)) 提供 Go 绑定。
* 另外一个: [gobbi](https://github.com/pekim/gobbi) (Requires CGO) (包括 [cairo](https://github.com/pekim/gobbi/tree/master/lib/cairo))。

[gotk4](https://github.com/diamondburned/gotk4) (Requires CGO) 是 Go 的 GTK4 绑定生成器。

[GoVCL](https://github.com/ying32/govcl) (Requires CGO) 是一个基于 [liblcl](https://github.com/ying32/liblcl)的本机 GUI 库。

[GXUI](https://github.com/google/gxui) 是一个 Go 跨平台 UI 库。

[iup-go](https://github.com/gen2brain/iup-go) (Requires CGO) 是 [IUP](https://www.tecgraf.puc-rio.br/iup/)的 Go 绑定

[MacDriver](https://github.com/progrium/macdriver) (Requires CGO) 是一个用于在 Go 中使用 Apple/Mac API 和框架的工具包。

[nuxui](https://github.com/nuxui/nuxui) (Requires CGO) 是一个跨平台的 GUI 库，用于制作 macOS、window、linux、IOS、android 应用程序。

[qt](https://github.com/therecipe/qt) (Requires CGO) 允许您完全用 Go 编写 Qt 应用程序，并使以后部署它们变得非常容易。
...
[systray](https://github.com/getlantern/systray) (Requires CGO) 是一个跨平台的 Go 库，用于在通知区域放置图标和菜单。已在 Windows 8、Mac OSX、Ubuntu 14.10 和 Debian 7.6 上测试。

[trayhost](https://github.com/shurcooL/trayhost) (Requires CGO) 是一个跨平台的 Go 库，用于在主机操作系统的任务栏中放置图标。

[ui](https://github.com/andlabs/ui) (Requires CGO) 旨在使用 c 库 [libui](https://github.com/andlabs/libui)，用 Go 提供简单的 GUI 软件开发。
...
[go-sciter](https://github.com/sciter-sdk/go-sciter) (Requires CGO) 是 Sciter 的 Golang 绑定：用于现代 UI 开发的可嵌入 HTML/CSS/脚本引擎。
...
[Muon](https://github.com/ImVexed/muon) (Requires CGO) 是 Electron 的轻量级替代品，用 Golang 编写，使用[Ultralight](https://ultralig.ht/)而不是 Chromium。

[Qlovaseed](https://github.com/qlova/seed) 是使用 Golang 编写的 Electron 的轻量级替代品。应用程序以 HTML 页面的形式呈现。它不捆绑浏览器。

[Wails](https://github.com/wailsapp/wails) (Requires CGO) 是一个使用 Go 和 Web 技术构建桌面应用程序的框架。

[webview](https://github.com/webview/webview) (Requires CGO) 是一个小型跨平台 webview 库，用于 C/C++/Golang 构建现代跨平台 GUI。

### 自定义 GUI

[duit](https://github.com/mjl-/duit) 是一个纯 go、跨平台、MIT 许可的开发人员 UI 工具包。

[Fyne](https://github.com/fyne-io/fyne) (Requires CGO) 是一个用 Go 编写的易于使用的 UI 工具包和应用程序 API。我们使用 OpenGL（通过 go-gl 和 go-glfw 项目）来提供跨平台图形。

[gi](https://github.com/goki/gi) (Requires CGO) 是 Go 中基于场景图的 2D 和 3D GUI/图形界面 (Gi)。
...
[go-flutter](https://github.com/go-flutter-desktop/go-flutter) (Requires CGO) 是一个将 Flutter 应用到桌面的包。

[gui](https://github.com/faiface/gui) 为 Go 中的并发 GUI 提供了极简、坚如磐石的基础。

[imgui-go](https://github.com/inkyblackness/imgui-go) (Requires CGO) 是 Dear ImGui 的 Go 包装器。
* [giu](https://github.com/AllenDang/giu) (Requires CGO) 是一个基于 imgui-go 的 GUI 框架。（另一个类似的框架来自同一作者， [gimu](https://github.com/AllenDang/gimu)，但适用于 nucular。）

[NanoGUI.go](https://github.com/shibukawa/nanogui-go) 是 [NanoGUI](https://github.com/wjakob/nanogui) 的 Go 端口

[nk](https://github.com/golang-ui/nuklear) (Requires CGO) 为 nuklear.h 提供 Go 绑定。
...
[gl](https://github.com/go-gl/gl) (Requires CGO) 存储库包含 Go 与各种 OpenGL 版本的绑定。它们是使用 Glow 自动生成的。
...
* 还有 [android-go](https://github.com/xlab/android-go) (Requires CGO) 项目中的 OpenGL ES 实现。

[vulkan](https://github.com/vulkan-go/vulkan) (Requires CGO) 为 Vulkan 提供 Go 绑定——一种低开销、跨平台的 3D 图形和计算 API。
...
[go-cairo](https://github.com/ungerik/go-cairo) (Requires CGO) 是一个基于 Gio 构建的画布 API。
* 以及上面提到的 [gotk3](https://github.com/gotk3/gotk3/tree/master/cairo) [gobbi](https://github.com/pekim/gobbi/tree/master/lib/cairo) [cairo](https://github.com/golang-ui/cairo) (Requires CGO).

[Go canvas](https://github.com/tfriedel6/canvas) (Requires CGO) 是一个 Go 库，提供与 HTML5 canvas API 尽可能相似的绘图功能，在 OpenGL 后端（和软件后端）上实现。
...
[go-skia](https://github.com/go101/go-skia) (Requires CGO) 是一个通过 cgo 基于 skia C 库的 Go skia 绑定。
...
* [NanoVG](https://github.com/memononen/nanovg)的 CGO 绑定版本[nanovgo](https://github.com/beta/nanovgo) (Requires CGO)。
...
[ebiten](https://github.com/hajimehoshi/ebiten) (Requires CGO)  一款非常简单的 Golang 2D 游戏库
* [ebitenui](https://github.com/ebitenui/ebitenui) 基于 ebiten 的 UI 库

[godot-go](https://github.com/ShadowApex/godot-go) (Requires CGO) 是 [Godot Engine](https://godotengine.org/)'s 的 [GDNative API](https://github.com/GodotNativeTools/godot_headers) 的 Go 绑定。

[raylib-go](https://github.com/gen2brain/raylib-go) (Requires CGO) 是 [raylib](https://www.raylib.com) 的 Go 绑定。
