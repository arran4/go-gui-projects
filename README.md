...
[DarwinKit](https://github.com/progrium/darwinkit) (Requires CGO) (old name: MacDriver) lets you work with Apple frameworks and build native Mac applications using Go.

[dlgs](https://github.com/gen2brain/dlgs) is a cross-platform library for displaying dialogs and input boxes.

[gamen](https://github.com/rajveermalviya/gamen) (Requires CGO) is cross-platform GUI window creation and management library in Go.
...
[glfw](https://github.com/go-gl/glfw) (Requires CGO) is a Go bindings for GLFW 3.

[go-fltk](https://github.com/pwiecz/go-fltk) (Requires CGO) is a simple wrapper around [FLTK 1.4 library](https://www.fltk.org/), which is a lightweight GUI library which allows creating small, self-contained and fast gui applications.

[go-mobile](https://github.com/golang/mobile) (Requires CGO) adds support for mobile platforms (Android and iOS) and provides tools to build mobile applications. (A [Go bindings for OpenGL ES 2.0 and ES 3.0](https://godoc.org/golang.org/x/mobile/gl) is included.)

[go-sfml](https://github.com/teh-cmc/go-sfml) (Requires CGO) is a Go bindings for [SFML](https://www.sfml-dev.org/).

[GoVCL](https://github.com/ying32/govcl) (Requires CGO) is a native GUI library based on [liblcl](https://github.com/ying32/liblcl).

GTK related:
* [go-gtk](https://github.com/mattn/go-gtk) (Requires CGO) is a Go binding for GTK2.
* [gotk3](https://github.com/gotk3/gotk3) (Requires CGO) provides Go bindings for GTK+3 and dependent projects (including [cairo](https://github.com/gotk3/gotk3/tree/master/cairo)).
  * Another one: [gobbi](https://github.com/pekim/gobbi) (Requires CGO) (including [cairo](https://github.com/pekim/gobbi/tree/master/lib/cairo)).
* [gotk4](https://github.com/diamondburned/gotk4) (Requires CGO) is a GTK4 bindings generator for Go.

[GXUI](https://github.com/google/gxui) is a Go cross platform UI library.

[iup-go](https://github.com/gen2brain/iup-go) (Requires CGO) is a Go bindings for [IUP](https://www.tecgraf.puc-rio.br/iup/).

[nuxui](https://github.com/nuxui/nuxui) (Requires CGO) is a cross-platform GUI library to make macOS, window, linux, IOS, android applications.

QT related:
* [miqt](https://github.com/mappu/miqt) (Requires CGO) is a MIT-licensed Qt bindings for Go.
* [Qamel](https://github.com/go-qamel/qamel) (Requires CGO) is a simple QML binding for Go (MIT license)
* [qt](https://github.com/therecipe/qt) (Requires CGO) allows you to write Qt applications entirely in Go and makes deploying them later very easy (LGPL license).
...
SDL related:
* [go-sdl2](https://github.com/veandco/go-sdl2) (Requires CGO) is SDL2 wrapped for Go users.
...
[systray](https://github.com/getlantern/systray) (Requires CGO) is a cross platfrom Go library to place an icon and menu in the notification area. Tested on Windows 8, Mac OSX, Ubuntu 14.10 and Debian 7.6.

Tcl/Tk related:
* [gothic](https://github.com/nsf/gothic) (Requires CGO) is a tcl/tk binding.
* [tk9.0](https://gitlab.com/cznic/tk9.0) is a CGo-free (use dynamic lib loading instead), cross platform GUI toolkit for Go.

[trayhost](https://github.com/shurcooL/trayhost) (Requires CGO) is a cross-platform Go library to place an icon in the host operating system's taskbar.

[ui](https://github.com/andlabs/ui) (Requires CGO) aims to provide simple GUI software development in Go, based on my c lib [libui](https://github.com/andlabs/libui).
...
[go-sciter](https://github.com/sciter-sdk/go-sciter) (Requires CGO) is a Golang bindings of Sciter: the Embeddable HTML/CSS/script engine for modern UI development。
...
[Muon](https://github.com/ImVexed/muon) (Requires CGO) is a lightweight alternative to Electron written in Golang, using [Ultralight](https://ultralig.ht/) instead of Chromium.
...
[Wails](https://github.com/wailsapp/wails) (Requires CGO) is a framework for building desktop applications using Go & Web Technologies.

[webview](https://github.com/webview/webview) (Requires CGO) is a tiny cross-platform webview library for C/C++/Golang to build modern cross-platform GUIs.

### custom GUI

[Cogent Core](https://github.com/cogentcore/core) (Requires CGO) is a free and open source framework for building powerful, fast, elegant 2D and 3D apps that run on macOS, Windows, Linux, iOS, Android, and the web.

[duit](https://github.com/mjl-/duit) is a pure go, cross-platform, MIT-licensed ui toolkit for developers.

[Fyne](https://github.com/fyne-io/fyne) (Requires CGO) is an easy to use UI toolkit and app API written in Go. We use OpenGL (through the go-gl and go-glfw projects) to provide cross platform graphics.
...
[go-flutter](https://github.com/go-flutter-desktop/go-flutter) (Requires CGO) is a package that brings Flutter to the desktop.

[gui](https://github.com/faiface/gui) provides super minimal, rock-solid foundation for concurrent GUI in Go.

[imgui-go](https://github.com/inkyblackness/imgui-go) (Requires CGO) is a Go wrapper for Dear ImGui.
* [giu](https://github.com/AllenDang/giu) (Requires CGO) is a GUI framework based on imgui-go. (Another similar one from the same auther, [gimu](https://github.com/AllenDang/gimu), but for nucular.)
...
[nk](https://github.com/golang-ui/nuklear) (Requires CGO) provides Go bindings for nuklear.h.
...
[gl](https://github.com/go-gl/gl) (Requires CGO) repository holds Go bindings to various OpenGL versions. They are auto-generated using Glow.
...
* Also the OpenGL ES implemention in the [android-go](https://github.com/xlab/android-go) (Requires CGO) project.

[vulkan](https://github.com/vulkan-go/vulkan) (Requires CGO) provides Go bindings for Vulkan — a low-overhead, cross-platform 3D graphics and compute API.
...
* [giocanvas](https://github.com/ajstarks/giocanvas) (Requires CGO) is a canvas API built on top of Gio.

[go-cairo](https://github.com/ungerik/go-cairo) (Requires CGO) is a Go binding for the cairo graphics library.
* And the above mentioned [the](https://github.com/gotk3/gotk3/tree/master/cairo) [other](https://github.com/pekim/gobbi/tree/master/lib/cairo) [ones](https://github.com/golang-ui/cairo) (Requires CGO).

[Go canvas](https://github.com/tfriedel6/canvas) (Requires CGO) is a pure Go library that provides drawing functionality as similar as possible to the HTML5 canvas API, implemented on OpenGL backend (and software backend).
...
[go-skia](https://github.com/go101/go-skia) (Requires CGO) is a Go skia binding based on skia C library through cgo.
...
* [A CGO binding version](https://github.com/beta/nanovgo) (Requires CGO) of NanoVG.
...
[ebiten](https://github.com/hajimehoshi/ebiten) (Requires CGO)  – A dead simple 2D game library for Golang
* [ebitenui](https://github.com/ebitenui/ebitenui) - A UI library on top of ebiten

[godot-go](https://github.com/ShadowApex/godot-go) (Requires CGO) - Go language bindings for the [Godot Engine](https://godotengine.org/)'s [GDNative API](https://github.com/GodotNativeTools/godot_headers).

[raylib-go](https://github.com/gen2brain/raylib-go) (Requires CGO) is a Golang binding for [raylib](https://www.raylib.com).
